...
namespace Graphs
{
    public class Graph
    {
        ...
        #region BFS_SEARCH
        public void BFSTraverse(Graph graph, string startNode)
        {
            BFS tempBFS = new BFS();

            tempBFS._BFSTraverse(graph, startNode);
        }

        public bool BFSConfirmPath(Graph graph, string startingNode, string goalNode)
        {
            BFS tempBFS = new BFS();

            return tempBFS._BFSConfirmPath(graph, startingNode, goalNode);
        }

        public List<string> BFSPathfind(Graph graph, string startingNode, string toNode)
        {
            BFS tempBFS = new BFS();

            return tempBFS._BFSPathfind(graph, startingNode, toNode);
        }
        #endregion

        #region DFS_SEARCH
        public void DFSTraverse(Graph graph, string startingNode)
        {
            DFS tempDFS = new DFS();

            tempDFS._DFSTraverse(graph, startingNode);
        }

        public bool DFSConfirmPath(Graph graph, string startingNode, string goalNode)
        {
            DFS tempDFS = new DFS();

            return tempDFS._DFSConfirmPath(graph, startingNode, goalNode);
        }

        public List<string> DFSPathfind(Graph graph, string startingNode, string goalNode)
        {
            DFS tempDFS = new DFS();

            return tempDFS._DFSPathfind(graph, startingNode, goalNode);
        }
        #endregion

        #region DFS_R_SEARCH
        public void DFSR_Traverse(Graph graph, string from)
        {
            DFS_R tempDFSR = new DFS_R();

            tempDFSR._DFSR_Traverse(graph, from);
        }

        public bool DFSRPathConfirm(Graph graph, string from, string goalNode)
        {
            DFS_R tempDFSR = new DFS_R();

            return tempDFSR._DFSR_PathConfirmation(graph, from, goalNode);
        }
